  ...
  static get template() {
    return html`
        <style>
            ...
            :host(:not([opened])) #label .labelFallback,
            ...
            :host([opened]) #label .labelFallback,
            ...
            <div id="main">
                ...
                <div id="chip">
                    ...
                    <div id="label">
                        ...
                        <slot id="labelSlot" name="label">
                            <!-- fallback -->
                            <div class="labelFallback">

                                <slot id="fallbackLabelSlot"></slot>
...
`;
  ...
  static get observers() {
      return [
          "_handleVisibleLabel(hasLabel, value)"
      ]
  }
  ...
  connectedCallback() {
      ...
      this._boundOnTap = (evt) => this._onTap(evt)
      this._boundOnBlur = (evt) => this._onBlur(evt)
      this._boundOnSlotChange = (evt) => this._onSlotChange(evt)
      ...
      // Detect whether we have a label somehow (at boot and also at runtime)
      this._onSlotChange()
      this.$.fallbackLabelSlot.addEventListener("slotchange", this._boundOnSlotChange)
      this.$.labelSlot.addEventListener("slotchange", this._boundOnSlotChange)
  }
  ...
  _onSlotChange(event) {
      if (event) {
          if (event.target == event.currentTarget) {
              switch (event.target) {
                  case this.$.labelSlot:
                      this._setHasLabel(this.$.labelSlot.assignedNodes().length > 0)
                      break

                  case this.$.fallbackLabelSlot:
                      this._setHasLabel(this.$.fallbackLabelSlot.assignedNodes().length > 0)
                      break
              }
          }
      ...
      this._setHasLabel(this.$.labelSlot.assignedNodes().length > 0 || this.$.fallbackLabelSlot.assignedNodes().length > 0)
  }

  _handleVisibleLabel(hasLabel, value) {
      if (!hasLabel && !!value && !this.textContent) {
          // Restarts (fallback) distribution
  ...
  get label() {
      ...
      var nodes = this.$.labelSlot.assignedNodes()
      if (nodes.length <= 0) {
          nodes = this.$.fallbackLabelSlot.assignedNodes()
      }
